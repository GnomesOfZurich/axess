...
use axess::{
    AuthSession, AuthnService, MemorySessionStore, MockFactorStore, MockIdentityStore,
    OAuthProviderConfig, SessionLayer,
};
use axum::{
    Router,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client_id = std::env::var("OAUTH_CLIENT_ID").expect("Set OAUTH_CLIENT_ID env var");
    let client_secret =
        std::env::var("OAUTH_CLIENT_SECRET").expect("Set OAUTH_CLIENT_SECRET env var");
    let issuer =
    ...
    let signing_key = [0u8; 32]; // Use a real key in production!
...
async fn login(
    ...
    let options = axess::OAuthLoginOptions::default();
    match state
        .authn
...
async fn profile(session: AuthSession) -> impl IntoResponse {
    if session.is_authenticated().await {
        let user_id = session.user_id().await.unwrap_or_else(|| "unknown".into());
        Html(format!(
            r#"<!doctype html>
